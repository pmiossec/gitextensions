...
Detail of the error:");
    ...
    private IObservable<IBuildInfo> GetBuilds(DateTime? sinceDate = null, bool running = false)
        => Observable.Create<IBuildInfo>((observer, cancellationToken) => ObserveBuildsAsync(sinceDate, running, observer));

    private async Task ObserveBuildsAsync(DateTime? sinceDate, bool running, IObserver<IBuildInfo> observer)
    {
